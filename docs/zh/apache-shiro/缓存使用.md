# **缓存使用**
> 使用 Shiro 进行权限管理过程中，每次授权都会访问 Realm 中的 doGetAuthorizationInfo 方法查询当前用户的角色及权限信息，如果系统的用户量比较大则会对数据库造成比较大的压力
>
> Shiro 支持缓存以降低对数据库的访问压力（缓存的是授权信息）
## **导入依赖**
---
- Shiro 支持缓存 使用第三方缓存
...
- `ShiroConfig.java` ——更改方法 `getDefaultWebSecurityManager(MyRealm myRealm)`
